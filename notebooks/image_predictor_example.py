...
# jupyter nbconvert --to python notebooks/image_predictor_example.ipynb
...
if using_colab:
    ...
    import sys
    import subprocess
    subprocess.run([sys.executable]'{sys.executable} -m pip install opencv-python matplotlib'.split(' '))
    subprocess.run([sys.executable]"{sys.executable} -m pip install 'git+https://github.com/facebookresearch/sam2.git'".split(' '))

    # IF images folder not exists
    if not os.path.exists('images'):
        subprocess.run('mkdir -p images'.split(' '))
        subprocess.run('wget -P images https://raw.githubusercontent.com/facebookresearch/sam2/main/notebooks/images/truck.jpg'.split(' '))
        subprocess.run('wget -P images https://raw.githubusercontent.com/facebookresearch/sam2/main/notebooks/images/groceries.jpg'.split(' '))
    # if checkpoints/sam2.1_hiera_large.pt not exists
    if not os.path.exists('../checkpoints/sam2.1_hiera_large.pt'):
        subprocess.run('mkdir -p ../checkpoints/'.split(' '))
        subprocess.run('wget -P ../checkpoints/ https://dl.fbaipublicfiles.com/segment_anything_2/092824/sam2.1_hiera_large.pt'.split(' '))
